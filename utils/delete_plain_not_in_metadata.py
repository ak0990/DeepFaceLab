import os
import sys
import re
import argparse
#import Path_utils
from core import pathex
from pathlib import Path
from DFLPNG import DFLPNG
from DFLJPG import DFLJPG
from tqdm import tqdm

if sys.version_info[0] < 3 or (sys.version_info[0] == 3 and sys.version_info[1] < 2):
    raise Exception("This program requires at least Python 3.2")

class fixPathAction(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        setattr(namespace, self.dest, os.path.abspath(os.path.expanduser(values)))

def str2bool(v):
    if v.lower() in ('yes', 'true', 't', 'y', '1'):
        return True
    elif v.lower() in ('no', 'false', 'f', 'n', '0'):
        return False
    else:
        raise argparse.ArgumentTypeError('Boolean value expected.')

def process_sort(arguments):
    from mainscripts import Sorter
    Sorter.main (input_path=arguments.input_dir, sort_by_method=arguments.sort_by_method)


def get_img_list(input_path):
    img_list = []
    print("Loading debug images")
    for filepath in tqdm(pathex.get_image_paths(input_path)):
        filepath = Path(filepath)

        if filepath.suffix != '.png' and filepath.suffix != '.jpg':
            print ("%s is not a image file" % (filepath.name) )
            continue

        img_list.append(str(filepath))

    return img_list

def get_aligned_img_list(input_path):
    print("Loading training images")
    img_list = []
    for filepath in tqdm(pathex.get_image_paths(input_path)):
        filepath = Path(filepath)

        if filepath.suffix != '.png' and filepath.suffix != '.jpg':
            print ("%s is not a image file" % (filepath.name) )
            continue

        if filepath.suffix == '.png':
            a_img = DFLPNG.load (str(filepath))
            if a_img is None:
                print ("%s failed to load" % (filepath.name) )
                continue
        elif filepath.suffix == '.jpg':
            a_img = DFLJPG.load (str(filepath))
            if a_img is None:
                print ("%s failed to load" % (filepath.name) )
                continue

        d = a_img.getDFLDictData()

        if d is None or d['source_filename'] is None:
            print ("%s - no embedded data found required for sort_by_origname" % (filepath.name) )
            continue

        img_list.append( [str(filepath), d['source_filename']] )

    return img_list

def remove_debug_not_in_train(debug_img_list, train_img_list):
    re_debug = re.compile(r'_debug')
    re_train = re.compile(r'_[0-9]+\.')
    re_source_name_format = re.compile(r'[0-9]+\.')
    re_train_name_format = re.compile(r'[0-9]+_*[0-9]\.')

    print("Validating filenames")
    validation = True
    for train_img in train_img_list:
        train_source_filename = train_img[1]
        train_filename = train_img[0]
        if not re_source_name_format.search(train_source_filename):
            print("Bad source name format - skipping: %s" %train_source_filename)
            validation = False
        if not re_train_name_format.search(train_filename):
            print("Bad train name format - skipping: %s" %train_filename)
            validation = False

    if validation == False:
        print("\nBad filename(s) found - exiting")
        return

    print("Formatting train filenames")
    train_img_list_formatted = []
    for train_img in train_img_list:
        train_filename = Path(train_img[0])
        train_orig = train_img[1]
        train_trimmed = re_train.sub('.', train_orig)
        train_trimmed = Path(train_trimmed).stem
        train_img_list_formatted.append(train_trimmed)

    print("Searching for files to remove")
    pbar = tqdm(debug_img_list)
    for debug_img in pbar:
        found = False
        debug_img_trimmed = os.path.basename(re_debug.sub('', debug_img))
        debug_img_trimmed = Path(debug_img_trimmed).stem
        if debug_img_trimmed in train_img_list_formatted:
                found = True
        else:
            #print("NOMATCH Debug: %-40s # Train: %-40s # Orig: %s" %(debug_img_trimmed, train_trimmed, train_orig))
            #print("REMOVING Debug: %-15s " %(debug_img))
            pbar.set_description("Removed: %s" %debug_img)
            os.remove(debug_img)


if __name__ == "__main__":
    print("This script requires all image filenames are of the format: ")
    print("  source: ^[0-9]+\.")
    print("  train:  ^[0-9]+_[0-9]." + '\n')
    try:
        debug_dir = sys.argv[1]
        train_dir = sys.argv[2]
    except:
        print("Usage: %s debug_img_path faces_to_train_path" %(sys.argv[0]))

    debug_img_list = get_img_list(debug_dir)
    train_img_list = get_aligned_img_list(train_dir)
    remove_debug_not_in_train(debug_img_list, train_img_list)
